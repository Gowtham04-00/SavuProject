      ...
      <ul>
        {notes.map((note) => (
          <li key={note.id}>
            ...
            <button className='btn btn-danger' type='submit' style={{ marginLeft: '20px', marginBottom:'25px'}}  onClick={() => deleteNote(note.id)}>
        Delete
        </button>
          </li>
        ))}
      </ul>
      <Link to={`/NotesAdd/${id}`} className='btn btn-success'>Add</Link>
    </div>
  );
};
