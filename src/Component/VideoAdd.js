  ...
  const fileHandler = (event) => {
    ...
    if (selectedFile) {
      ...
      if (selectedFile.type.startsWith('video/')) {
        // File is a video, proceed with setting the file
        setFile(selectedFile);
      } else {
        // File is not a video, show an error or handle it accordingly
  ...
  const addData = (event) => {
    ...
    axios.post("https://localhost:7028/api/module/uploadvideo", formData)
      ...
      .catch((err) => {
        ...
        window.alert("Error uploading video. Please try again.");
      });
  };
